...
def run_against_set(adv_csv_path: str, model, tokenizer) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        inputs = tokenizer(row["text"], truncation=True, max_length=128, return_tensors="pt")
        with torch.no_grad():
            logits = model(**inputs).logits
        probs = torch.softmax(logits, dim=-1)
        argmax = torch.argmax(probs, dim=-1).item()
        label = model.config.id2label[argmax]
        prob = float(probs[0, argmax])
        results.append({
            **row.to_dict(),
